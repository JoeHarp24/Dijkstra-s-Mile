...
*   `src/graph.js`: Defines the structure of the graph (nodes and edges). *This file is designed to be populated dynamically from GeoJSON.*
...
| **Algorithm** | Dijkstra's | Optimal for finding the single-source shortest path in a graph with non-negative edge weights (which elevation differences will be). | Ensures the *absolute* minimum weighted path is found. |
